...
int skaiciuotiStudentus(const string& failoPavadinimas) {
    ifstream file(failoPavadinimas);
    return std::count(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>(), '\n');
}
void testuotiDuomenuApdorojima(const string& failoPavadinimas, const string& rezultataiAplankas) {
    
    // Skaičiuojame studentų skaičių faile
    int skaicius = skaiciuotiStudentus(failoPavadinimas);
    ...
    vector<studentas<vector<float>>> vargsiai, kietiakai;
    for (auto& stud : grupe) {
        if (stud.Gal < 5.0) {
